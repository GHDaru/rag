"""harness-zero — Etapa 8: plan mode. Pensar antes de agir — imposto, não pedido.

Peça a um agente "planeje antes de executar" no prompt e ele vai... executar
mesmo assim, de vez em quando. A lição do capítulo 09, aprendida pelos
harnesses reais (Claude Code plan mode, gemini-cli): **modo de planejamento
não é um pedido educado — é uma POLÍTICA imposta pelas permissões.**

Esta etapa adiciona um MODO por sessão:

    executar  — tudo como na etapa 7
    planejar  — a política NEGA toda ferramenta mutante; o agente só pode
                LER e usar a nova tool `propor_plano` (que escreve o
                artefato PLAN — o plano vira ARQUIVO, revisável)

A mudança na política é UMA linha: `decide()` agora recebe o modo. É o
ponto da etapa: quem garante o comportamento é o mesmo mecanismo do cap. 07
— permissões — e não a boa vontade do modelo. Aprovar o plano (humano) é
trocar o modo de volta para executar.

Rodar:  uvicorn app:app --reload  →  alterne o modo no chat; em "planejar",
peça "escreva notas.txt" (negado) e "proponha um plano para X" (vira PLAN).
Janela: GET /plano?session_id=…
"""
...
app = FastAPI(title="harness-zero · etapa 8")
