"""harness-zero — Etapa 4: sessões. O histórico sai da variável global.

Desde a etapa 0 carregamos uma dívida de propósito: `history` era uma LISTA
GLOBAL. Reinicie o servidor e a conversa evapora; abra duas abas e elas se
misturam. Essa é a dor que o capítulo 08 (Memória e Estado) resolve.

Nasce a terceira porta: o **StorePort** — quem guarda e devolve mensagens.
Dois adapters, mesma interface:

    MemoriaStore  — a lista de sempre (útil em testes; a dor visível)
    SQLiteStore   — persistência real num arquivo .db ao lado do app

E o conceito de **sessão**: cada conversa tem um `session_id`; o chat manda
o seu; `/sessions` lista as existentes; retomar = mandar o mesmo id (é o
`/resume` dos harnesses reais — Claude Code `--resume`, opencode sessions).

Repare no padrão que se repete (etapas 2, 3 e agora 4): a dor vira uma
PORTA, o loop não muda, e um endpoint vira a janela de observação.
O companion do livro roda exatamente esta arquitetura em produção
(com Postgres no lugar do SQLite — mesmo StorePort, outro adapter).
...
app = FastAPI(title="harness-zero · etapa 4")
