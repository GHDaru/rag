"""harness-zero — Etapa 11: hooks. Estender sem tocar no loop.

O harness já tem loop, tools, contexto, sessões, compactação, permissões,
MCP e subagentes. E quando alguém precisar de um comportamento NOVO —
auditar cada chamada, mascarar segredos nos resultados, bloquear um padrão
da empresa? Reescrever o loop a cada necessidade é o anti-padrão que o
capítulo 12 ataca. A resposta dos harnesses reais: **hooks** — pontos de
extensão em fronteiras estáveis.

Esta etapa adiciona DUAS fronteiras na execução de ferramentas:

    pre_tool(nome, args)              → pode BLOQUEAR ou AJUSTAR os args
    post_tool(nome, args, resultado)  → pode TRANSFORMAR o resultado

E dois hooks de exemplo que resolvem dores reais:
    auditoria  — cada chamada vira uma linha em auditoria.jsonl (quem/quando/o quê)
    redator    — resultados passam por um filtro que MASCARA padrões de
                 segredo (nvapi-…, password=…) antes de chegarem ao modelo

Repare: o LOOP NÃO MUDOU. Os hooks envolvem `registro.executar` — a mesma
lição das portas, agora para terceiros: fronteira estável, comportamento
plugável. (No mundo real: hooks do Claude Code, extensões do gemini-cli.)

Rodar:  uvicorn app:app --reload  →  depois `cat auditoria.jsonl`
"""
...
app = FastAPI(title="harness-zero · etapa 11")
