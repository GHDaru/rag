"""harness-zero — Etapa 9: subagentes. Dividir para conquistar — com isolamento.

Tarefas grandes estouram uma conversa só: o contexto lota (etapa 5 atenua,
não resolve) e o modelo perde o fio. O capítulo 10 mostra o padrão que todos
os harnesses maduros adotaram: **delegar para um subagente com contexto
LIMPO** — a tool `task` do Claude Code, os sub-agents do gemini-cli.

Esta etapa adiciona a tool `task(descricao)`. Quando o modelo a chama:

    1. nasce uma SESSÃO-FILHA ("task-…"), com histórico próprio e
       system prompt focado só na descrição — ZERO contexto do pai;
    2. a filha roda o MESMO loop, com ferramentas RESTRITAS
       (só leitura — filha não muta o mundo; quem muta é o pai, com
       aprovação humana; a política da etapa 6 continua por cima);
    3. termina, e SÓ O RESULTADO FINAL volta ao pai como tool result.

As duas fronteiras são a lição: **na ida, só a descrição; na volta, só o
resultado.** É isso que mantém o contexto do pai limpo — o motivo de
existir subagente. As filhas ficam visíveis em /sessions (são sessões
como quaisquer outras — a etapa 4 pagando dividendos).

Rodar:  uvicorn app:app --reload  →  "use task para resumir o AGENTS.md"
"""
...
def retomar(pid: str, aprovado: bool) -> dict:
    ...
    for j, call in enumerate(p["calls_restantes"]):
        ...
        if j == 0:
            ...
            resultado = registro.executar(nome, args) if decide(nome, args, modo_de(p["session_id"])) == "permitir" \
...
app = FastAPI(title="harness-zero · etapa 10")
