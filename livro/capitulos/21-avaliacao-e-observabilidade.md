...
"O RAG não está bom" tem pelo menos quatro causas distintas, e a separação mínima é entre **achar** e **usar**.

1. **Leve a tabela de diagnóstico.** Recall baixo = não acha (chunking/busca); recall alto + precision baixa = traz lixo junto (reranking, `top_k` menor); as duas altas + *faithfulness* baixa = **tem tudo e inventa** (prompt de fundamentação). Sem ela, "melhorar o RAG" é tentativa e erro caro.
2. **Desconfie do caso que parece bom.** *Faithfulness* baixa numa resposta **factualmente correta** não é defeito da métrica: o modelo respondeu de memória, e não há garantia nenhuma sobre a próxima pergunta.
3. **Conjunto sintético superestima o recall** — a pergunta gerada de um trecho é respondível por aquele trecho. Pergunta real com resposta verificada por gente é insubstituível. E **nunca** gere as perguntas e julgue as respostas com o mesmo modelo que responde: isso mede consistência, não qualidade.
4. **Do eval ao painel, monitore três sinais:** taxa de **resultado zero** (se sobe, algo mudou; se vive em zero, não há limiar), **p99** (é onde o laço agêntico do cap. 18 aparece, não na mediana) e **taxa de citação**, que é *faithfulness* na versão barata, sem juiz.
5. **Saiba o que ainda não se mede bem:** trajetória, conversa (não só turno), custo ao lado da qualidade, e deriva.
