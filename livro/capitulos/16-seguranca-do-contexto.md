...
- **Contaminação de memória** — [arXiv 2605.28009](https://arxiv.org/abs/2605.28009): a memória de longo prazo como vetor de persistência do ataque. É o cap. 12 encontrando este. `[a validar]`
...
4. **Privilégio mínimo nas ferramentas** (cap. 14). A camada que muda a natureza do problema: se o modelo for convencido, o que ele consegue fazer? Uma ferramenta somente-leitura limita o dano de forma que não depende do modelo resistir.
...
Uma injeção que consegue gravar na memória (cap. 12) deixa de ser um incidente e vira uma condição: a afirmação falsa é recuperada em toda sessão futura, e o rastro do ataque original desaparece.
