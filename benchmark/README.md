...
O livro afirma coisas como "busca híbrida é o upgrade de melhor relação benefício/esforço" (cap. 06) e "*contextual retrieval* e *late chunking* resolvem o mesmo problema com contas muito diferentes" (cap. 09). Hoje essas afirmações se apoiam em medições **de terceiros**, feitas em corpus de terceiros — e o Princípio I obriga a dizer isso a cada vez.
...
**Avalia-se:** técnicas comparáveis sobre o mesmo corpus, com a mesma pergunta e o mesmo modelo. Exemplos: busca densa × esparsa × híbrida; com e sem reranking; *contextual retrieval* × *late chunking*; estratégias de chunking; famílias de raciocínio (cap. 12); otimizadores de prompt (cap. 16).
...
|---|---|---|
| **Eficácia** | resolve o problema que promete? | métrica do estágio (cap. 21): recall, precision, faithfulness |
...
A nota **nunca** é comparável entre dimensões diferentes, e só é comparável entre técnicas que atacam o **mesmo problema** (as quatro falhas do cap. 09). Comparar *contextual retrieval* com reescrita de consulta é erro de categoria: elas curam falhas diferentes.
...
- **Perguntas sintéticas superestimam o recall** (cap. 21) — a pergunta gerada de um trecho é respondível por aquele trecho.
